import {ScrollView, Dimensions,KeyboardAvoidingView, SafeAreaView, View, Button, TouchableOpacity, Text, StyleSheet, Image, FlatList, Animated} from 'react-native';
...
const RecentlyViewed = ({navigation}) => {
  ...
  useEffect(() => {
    ...
    try{
      fetch("http://"+ip+":3000/projectsOrederByView")
      .then((resp) => resp.json())
  ...
  const getProject = (item) =>{
    ...
    if(item.date != '' && item.date != null && item.user_id == id ){
      myproject = true
    }
  ...
  const getResult = ( item ) => {
    ...
    projects.map(post=>{
      ...
      if(post.user_id == item.user_id ){
        ...
        if(item.name.startsWith(term) && item.date != '' && item.date != null && item.user_id == id ){
          Post = post
          t= true
        }else 

        if(item.name!=null && item.name.toLowerCase().startsWith(term.toLowerCase()) && item.date != '' && item.date != null && item.user_id == id ){
          Post = post
          t= true
        }
      }  
      ...
      <SafeAreaView style={styles.contentContainer}>
        ...
        <View style={styles.logo}>
          ...
          <View style={{alignSelf:'center', marginStart:-5}}>
            <Text style={{fontWeight:'700', fontSize:22}}></Text> 
          </View>
        ...
        <View style={{ marginTop:10, flexDirection:'row', justifyContent:'space-between'}}>
          ...
          {!showSearchList && <Text style={{fontSize:22, fontWeight:'500'}}> Recently Opened: </Text>}
          {!showSearchList &&<FontAwesome name="refresh" size={24} color="#555" style={{position:'absolute', left:200, marginTop:5}} onPress={()=>{setupdate(prevState => !prevState)}}/>}
